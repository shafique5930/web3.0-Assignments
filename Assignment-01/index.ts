...
while (loop) {
  if (answer.options == "groceries") {
    const grocers: { groceries: string; addmore: boolean } =
    ...
    const fruits: { Fruits: string; addmore: boolean } = await inquirer.prompt([
      {
        type: "list",
        name: "Fruits",
    ...
    const veggies: { Vegetables: string; addmore: boolean } =
      await inquirer.prompt([
        {
          type: "list",
          name: "Vegetables",
...
for (let x in shoppingCart) {
  ...
  let value: any = shoppingCart[x].slice(trim + 1);
  grandTotal.push(parseInt(value));
}
grandTotal.forEach((num) => {
  sum += num;
});
let discounted: number = (sum * 5) / 100;
if (sum > 100) {
  sum = sum - discounted;
}
...
if (checkOut.PaymentMethod == "POS") {
  ...
  console.log(
    `Please deposit a sum of Rs.${sum} to our EasyPaisa Account Number +923321235456 `
  );
...
let convertCelsiusToFahrenheit = (celciusTemp: number) => {
